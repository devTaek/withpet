...
const MyPage = () => {
  ...
  const [ isEditingPet, setIsEditingPet ] = useState<{mode: 'edit' | 'add' | ''}>({mode: ''});

  const handleUserEditClick = () => {
    setIsEditingUser(!isEditingUser);
  };
  const handlePetEditClick = (mode: 'edit' | 'add' | '') => {
    setIsEditingPet({mode: mode}); 
  };
    ...
    <div className="h-[calc(100vh-4rem)] mt-16 flex items-center justify-start m-20 gap-10 px-16">
      ...
      <div className="relative w-full h-4/5 md:w-1/3 bg-gray-100 p-6 rounded-lg shadow-md">
        <button className="absolute top-2 right-2 text-gray-500 hover:text-gray-800" onClick={handleUserEditClick}>
          <LiaUserEditSolid className='w-6 h-6' />
            ...
            { isEditingUser ? <EditUser setIsEditingUser={setIsEditingUser} user={user} />
              :
      ...
      <div className="relative w-full h-4/5 md:w-2/3 flex items-center justify-center flex-wrap gap-8 mt-6 md:mt-0">
          ...
          <button
            ...
            onClick={() => handlePetEditClick('add')}
            >
        ...
        <div className="relative w-full md:w-2/3 lg:w-1/2 bg-white p-6 rounded-lg shadow-md flex flex-col items-center">
          ...
          <button
            className="absolute top-2 right-2 text-gray-500 hover:text-gray-800"
            onClick={() => handlePetEditClick('edit')}
          >
            <TbMoodEdit className="w-6 h-6" />
          </button>
          {isEditingPet.mode === 'edit' || isEditingPet.mode === 'add' ?
            ( <EditPet isEditingPet={isEditingPet} setIsEditingPet={setIsEditingPet} user={user} /> )
            :
            (
                ...
                {user?.pet.map((pet: PetInfo, index: number) => (
                  <li key={index} className="w-full space-y-3">
                    ...
                    <div className="flex justify-between items-center border-b pb-2">
                      ...
                      <span className="text-lg text-gray-700">{pet.petName}</span>
                    </div>
                    {/* 견종 */}
                    <div className="flex justify-between items-center border-b pb-2">
                      <span className="text-lg font-semibold">견종</span>
                      <span className="text-lg text-gray-700">{pet.petSpecies}</span>
                    </div>
                    {/* 생일 */}
                    <div className="flex justify-between items-center border-b pb-2">
                      <span className="text-lg font-semibold">생일</span>
                      <span className="text-lg text-gray-700">{pet.petBirth}</span>
                    </div>
                    {/* 성별 */}
                    <div className="flex justify-between items-center border-b pb-2">
                      <span className="text-lg font-semibold">성별</span>
                      <span className="text-lg text-gray-700">{pet.petGender}</span>
                    </div>
                    {/* 성격 */}
                    <div className="flex justify-between items-center border-b pb-2">
                      <span className="text-lg font-semibold">몸무게</span>
                      <span className="text-lg text-gray-700">{pet.petWeight}</span>
                    </div>
                    {/* 음식 */}
                    <div className="flex justify-between items-center border-b pb-2">
                      <span className="text-lg font-semibold">좋아하는 음식</span>
                      <span className="text-lg text-gray-700">{pet.petFood}</span>
                    </div>
                    {/* 활동 */}
                    <div className="flex justify-between items-center border-b pb-2">
                      <span className="text-lg font-semibold">활동</span>
                      <span className="text-lg text-gray-700">{pet.petActivity}</span>
                    </div>
                  </li>
                ))}
                {/* 강아지 정보 */}
